...
export default class ResetPassword extends Component{
    ...
    constructor(){
        ...
        this.state={
            ...
            tabIndex:0,
            message:[
                "이메일을 입력하면 임시 비밀 번호를 보내드립니다.",
                "전화번호를 입력하면 임시 비밀 번호를 보내드립니다."
            ]
        }
    }
                    ...
                    <View style={styles.tabContainer}>
                        {
                            this.state.tabs.map((value, index)=>{
                                return (
                                    <TabComponent label={value} key={"Tab"+index} selected={this.state.tabIndex==index} onPress={()=>this.setTabIndex(index)}></TabComponent>
                                );
                            })
                        }
                    </View>
